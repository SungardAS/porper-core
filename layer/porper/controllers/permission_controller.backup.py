...
class PermissionController:
    ...
    def __init__(self, connection):
        ...
        self.PERMITTED_TO_ALL = '*'     # can be used only for 'user_id', 'group_id', 'action' & 'value'
    ...
    def is_group_admin(self, user_id, group_id):
        if group_id == self.PERMITTED_TO_ALL:
            rows = self.user_group.find({'user_id': user_id})
    ...
    def add_permissions_to_group(self, resource_name, resource_id, permissions, to_group_id):
        ...
        #       "condition": "{'is_admin': true|false}" ]   # valid only when 'to_group_id' is given
        #    }
        #    ...
        #]
    ...
    def _filter_conditions(self, user_id, rows):
        ...
        print("number of permissions without 'condition' is {}".format(len(filtered)))
        if len(filtered) == len(rows):
            # there is no row with a condition
            print("There is no permission with 'condition', so return all")
            return rows

        permissions = [ row for row in rows if row.get('condition') ]
        print("number of permissions with 'condition' is {}".format(len(permissions)))
        for permission in permissions:
            ...
            if 'is_admin' in condition:
                ...
                or ( permission.get('group_id') and self.is_group_admin(user_id, permission['group_id']) ):
                    print("permission with condition is valid, {}".format(permission))
    ...
    def find(self, access_token, params):
        ...
        possible attributes in params
            - None
            - [user_id | group_id], action, resource, [value], [condition]
               # 'condition' is valid only when 'user_id' is given
        """
