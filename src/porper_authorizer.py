...
def lambda_handler(event, context):
    ...
    if token is None:
        ...
        access_tokens = token_controller.find({'access_token': token})
        if len(access_tokens) == 0:
            error = 'Unauthorized'
        else:
            refresh_token = access_tokens[0]['refresh_token']
            user_guid = access_tokens[0]['user_id']
        principalId = 'user|a1b2c3d4'

    print("refresh_token: %s, user guid : %s" % (refresh_token, user_guid))
